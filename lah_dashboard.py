...
def getBudgets(userID, year=None):
    ...
    budgets_query = lah_budgets.getBudgets(userID)
...
# Gets the last four weeks start dates and end dates (e.g. '2023-04-13' and '2023-04-19')
...
def getSpendingTrends(userID, year=None):
    ...
    results = db.execute(text("SELECT category, COUNT(category) as count, SUM(amount) as amount FROM expenses WHERE user_id = :usersID AND date_part('year', date(expensedate)) = :year GROUP BY category ORDER BY COUNT(category) DESC"),
                         {"usersID": userID, "year": year}).fetchall()
