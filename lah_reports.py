...
def generateBudgetsReport(userID, year=None):
    ...
    budgetsReport = lah_dashboard.getBudgets(userID, year)

    # Loop through the budgets and add a new key/value pair to hold expense details per budget
    if budgetsReport:
        for record in budgetsReport:
            budgetID = lah_budgets.getBudgetID(record["name"], userID)
            results = db.execute(text("SELECT expenses.description, expenses.category, expenses.expenseDate, expenses.payer, expenses.amount FROM expenses WHERE user_id = :usersID AND date_part('year', date(expensedate)) = :year AND category IN (SELECT categories.name FROM budgetcategories INNER JOIN categories on budgetcategories.category_id = categories.id WHERE budgetcategories.budgets_id = :budgetID)"),
...
def generateMonthlyReport(userID, year=None):
    ...
    spending_month_chart = lah_dashboard.getMonthlySpending(userID, year)
...
def generateSpendingTrendsReport(userID, year=None):
    ...
    spending_trends_chart = lah_dashboard.getSpendingTrends(userID, year)
    ...
    categories_active = lah_categories.getSpendCategories(userID)
    ...
    categories_inactive = lah_categories.getSpendCategories_Inactive(userID)
    ...
    # Place a deep copy of the categories into each month (need deep copy here because every category may have unique spend data month to month.
    ...
    # Calculates and stores the amount spent per category for the table 
...
def generatePayersReport(userID, year=None):
    ...
    results_payers = db.execute(text(
    ...
    results_nonExpensePayers = db.execute(text(
