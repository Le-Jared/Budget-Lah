    ...
    for key, value in formData:
        ...
        if counter <= 3:
            ...
            # Check if year is valid
                ...
                if 2023 <= budgetYear <= currentYear:
...
def createBudget(budget, userID):
    ...
    newBudgetID = db.execute(text("INSERT INTO budgets (name, year, amount, user_id) VALUES (:budgetName, :budgetYear, :budgetAmount, :usersID) RETURNING id"),
...
def addCategory(budgetID, categoryIDS):
    ...
    for categoryID in categoryIDS:
        db.execute(text("INSERT INTO budgetCategories (budgets_id, category_id, amount) VALUES (:budgetID, :categoryID, :percentAmount)"),
...
def updateBudget(oldBudgetName, budget, userID):
    ...
    db.execute(text("UPDATE budgets SET name = :budgetName, year = :budgetYear, amount = :budgetAmount WHERE id = :oldBudgetID AND user_id = :usersID"),
    ...
    db.execute(text("DELETE FROM budgetCategories WHERE budgets_id = :oldBudgetID"),
...
def getBudgetCategoryIDS(categories, userID):
    ...
    for category in categories:
        # Get the category ID
        categoryID = db.execute(text("SELECT categories.id FROM userCategories INNER JOIN categories ON userCategories.category_id = categories.id WHERE userCategories.user_id = :usersID AND categories.name = :categoryName"),
...
def deleteBudget(budgetName, userID):
    ...
    if budgetID:
        ...
        db.execute(text("DELETE FROM budgetCategories WHERE budgets_id = :budgetID"),
                   {"budgetID": budgetID})
        db.commit()

        # Delete the budget
        db.execute(text("DELETE FROM budgets WHERE id = :budgetID"),
                   {"budgetID": budgetID})
        db.commit()
...
def getBudgetID(budgetName, userID):
    ...
    budgetID = db.execute(text("SELECT id FROM budgets WHERE user_id = :usersID AND name = :budgetName"),
...
def getUpdatableBudget(budget, userID):
    ...
    categories = lah_categories.getSpendCategories(userID)

    # Get the budget's spend categories and % amount for each category
    results = db.execute(text("SELECT DISTINCT categories.name, budgetCategories.amount FROM budgetCategories INNER JOIN categories ON budgetCategories.category_id = categories.id INNER JOIN budgets ON budgetCategories.budgets_id = budgets.id WHERE budgets.id = :budgetsID"),
