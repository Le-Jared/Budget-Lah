...
def getSpendCategories(userID):
    results = db.execute(text(
        ...
        params={"usersID": userID}).fetchall()
...
def getSpendCategoryLibrary():
    results = db.execute(text("SELECT id, name FROM categories")).fetchall()
...
def getSpendCategoryName(categoryID):
    name = db.execute(text("SELECT name FROM categories WHERE id = :categoryID"), {"categoryID": categoryID}).fetchone()[0]
...
def getBudgetsSpendCategories(userID):
    results = db.execute(text("SELECT budgets.name AS budgetname, categories.id AS categoryid, categories.name AS categoryname FROM budgetcategories INNER JOIN budgets on budgetcategories.budgets_id = budgets.id INNER JOIN categories on budgetcategories.category_id = categories.id WHERE budgets.user_id = :usersID ORDER BY budgets.name, categories.name"),
                         {"usersID": userID}).fetchall()
...
def getBudgetsFromSpendCategory(categoryID, userID):
    results = db.execute(text("SELECT budgets.id AS budgetid, budgets.name AS budgetname, categories.id AS categoryid, categories.name AS categoryname FROM budgetcategories INNER JOIN budgets on budgetcategories.budgets_id = budgets.id INNER JOIN categories on budgetcategories.category_id = categories.id WHERE budgets.user_id = :usersID AND budgetcategories.category_id = :categoryID ORDER BY budgets.name, categories.name"), {
...
def updateSpendCategoriesInBudgets(budgets, oldCategoryID, newCategoryID):
    for budget in budgets:
        ...
        db.execute(text("UPDATE budgetcategories SET category_id = :newID WHERE budgets_id = :budgetID AND category_id = :oldID"),
...
def deleteSpendCategoriesInBudgets(budgets, categoryID):
    for budget in budgets:
        ...
        db.execute(text("DELETE FROM budgetcategories WHERE budgets_id = :budgetID AND category_id = :categoryID"),
...
def addCategory_User(categoryID, userID):
    db.execute(text("INSERT INTO usercategories (user_id, category_id) VALUES (:usersID, :categoryID)"),
...
def deleteCategory_User(categoryID, userID):
    db.execute(text("DELETE FROM usercategories WHERE user_id = :usersID AND category_id = :categoryID"),
               {"usersID": userID, "categoryID": categoryID})
    db.commit()
...
def updateExpenseCategoryNames(oldCategoryName, newCategoryName, userID):
    db.execute(text("UPDATE expenses SET category = :newName WHERE user_id = :usersID AND category = :oldName"),
